...
class _PermintaanPageState extends State<PermintaanPage> {
  ...
  Future<void> _loadSavedData() async {
    ...
    final savedData = prefs.getString('permintaan_data');
    if (savedData != null) {
      setState(() {
  ...
  Future<void> _saveData() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('permintaan_data', json.encode(items));
  }
  ...
  Future<void> _downloadExcel() async {
    try {
      ...
      var sheet = excelFile['Permintaan Barang'];
      ...
      final anchor = html.AnchorElement(href: url)
        ..setAttribute("download", "Permintaan_Barang_${DateTime.now().millisecondsSinceEpoch}.xlsx")
        ..click();
      html.Url.revokeObjectUrl(url);

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('File berhasil disimpan di: Permintaan_Barang_${DateTime.now().millisecondsSinceEpoch}.xlsx')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error: ${e.toString()}')),
